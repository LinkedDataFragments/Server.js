/*! @license MIT ©2015-2016 Ruben Verborgh, Ghent University - imec */
let RdfView = require('../../lib/views/RdfView').RdfView,
    View = require('../../lib/views/View').View,
    N3 = require('n3'),
    PassThrough = require('stream').PassThrough;

const dataFactory = N3.DataFactory;

describe('RdfView', () => {
  describe('The RdfView module', () => {
    it('should be a function', () => {
      RdfView.should.be.a('function');
    });

    it('should be a View subclass', () => {
      (RdfView.prototype instanceof View).should.be.true;
    });
  });

  describe('An RdfView instance', () => {
    describe('without a _generateRdf implementation', () => {
      it('should throw an error when calling _generateRdf', () => {
        (function () { new RdfView(undefined, { dataFactory })._generateRdf({}, noop, noop, noop); })
          .should.throw('The _generateRdf method is not yet implemented.');
      });
    });

    describe('_renderViewExtension', () => {
      it('should call _generateRdf on an extension that implements it', () => {
        let extension = { _generateRdf: sinon.spy() },
            view = new RdfView(undefined, { dataFactory }),
            options = { writer: { data: noop, meta: noop } }, done = noop;
        view._renderViewExtension(extension, options, {}, {}, done);
        extension._generateRdf.should.have.been.calledOnce;
        extension._generateRdf.should.have.been.calledWith(options, options.writer.data, options.writer.meta, done);
      });

      it('should do nothing for an extension that does not implement _generateRdf', () => {
        let extension = {},
            view = new RdfView(undefined, { dataFactory }),
            options = { writer: { data: noop, meta: noop } };
        (function () { view._renderViewExtension(extension, options, {}, {}, noop); }).should.not.throw();
      });
    });

    describe('_addDatasources', () => {
      it('should skip datasources without a url', () => {
        let view = new RdfView(undefined, { dataFactory }), metadata = sinon.spy();
        view._addDatasources({ datasources: { a: { title: 'no-url datasource' } } }, noop, metadata);
        metadata.should.not.have.been.called;
      });
    });

    describe('_createN3Writer', () => {
      it('should preserve a metadata quad\'s own non-default graph', (done) => {
        let view = new RdfView(undefined, { dataFactory }), written = '';
        let response = { write: (data) => { written += data; } };
        let writer = view._createN3Writer(
          { contentType: 'application/trig', metadataGraph: 'urn:meta', fragmentUrl: 'urn:frag', prefixes: {} },
          response, () => {
            written.should.contain('urn:owngraph');
            done();
          });
        writer.meta(dataFactory.quad(
          dataFactory.namedNode('urn:s'), dataFactory.namedNode('urn:p'), dataFactory.namedNode('urn:o'),
          dataFactory.namedNode('urn:owngraph')));
        writer.end();
      });

      it('should write nothing when the underlying N3 writer errors', (done) => {
        let originalEnd = N3.Writer.prototype.end;
        N3.Writer.prototype.end = function (callback) { callback(new Error('failed')); };
        try {
          let view = new RdfView(undefined, { dataFactory }), written;
          let response = { write: (data) => { written = data; } };
          let writer = view._createN3Writer({ contentType: 'text/turtle', prefixes: {} }, response, () => {
            written.should.equal('');
            done();
          });
          writer.end();
        }
        finally { N3.Writer.prototype.end = originalEnd; }
      });
    });

    describe('_createJsonLdWriter', () => {
      function collect(view, settings) {
        let response = new PassThrough(), output = '';
        response.on('data', (d) => { output += d; });
        return { response, getOutput: () => output };
      }

      it('should not set @base when the prefixes have no base entry', (done) => {
        let view = new RdfView(undefined, { dataFactory }), { response, getOutput } = collect();
        let writer = view._createJsonLdWriter({ prefixes: {} }, response, () => {
          JSON.parse(getOutput())['@context'].should.not.have.property('@base');
          done();
        });
        writer.end();
      });

      it('should set @base when the prefixes have a base entry', (done) => {
        let view = new RdfView(undefined, { dataFactory }), { response, getOutput } = collect();
        let writer = view._createJsonLdWriter({ prefixes: { '': 'http://example.org/' } }, response, () => {
          JSON.parse(getOutput())['@context'].should.have.property('@base', 'http://example.org/');
          done();
        });
        writer.end();
      });

      it('should preserve a metadata quad\'s own non-default graph', (done) => {
        let view = new RdfView(undefined, { dataFactory }), { response, getOutput } = collect();
        let writer = view._createJsonLdWriter({ prefixes: {} }, response, () => {
          getOutput().should.contain('urn:owngraph');
          done();
        });
        writer.meta(dataFactory.quad(
          dataFactory.namedNode('urn:s'), dataFactory.namedNode('urn:p'), dataFactory.namedNode('urn:o'),
          dataFactory.namedNode('urn:owngraph')));
        writer.end();
      });

      it('should use the default graph for default-graph metadata when no metadataGraph is set', (done) => {
        let view = new RdfView(undefined, { dataFactory }), { response, getOutput } = collect();
        let writer = view._createJsonLdWriter({ prefixes: {}, metadataGraph: undefined }, response, () => {
          let parsed = JSON.parse(getOutput());
          parsed['@graph'][0].should.have.property('@id', 'urn:s');
          done();
        });
        writer.meta(dataFactory.quad(
          dataFactory.namedNode('urn:s'), dataFactory.namedNode('urn:p'), dataFactory.namedNode('urn:o'),
          dataFactory.defaultGraph()));
        writer.end();
      });

      it('should call back with an error when the underlying JSON-LD serializer errors', (done) => {
        let view = new RdfView(undefined, { dataFactory }), { response } = collect(), callCount = 0;
        let writer = view._createJsonLdWriter({ prefixes: {} }, response, (error) => {
          if (callCount++ === 0) {
            error.message.should.contain('Invalid JSON literal');
            done();
          }
        });
        writer.data(dataFactory.quad(
          dataFactory.namedNode('urn:s'), dataFactory.namedNode('urn:p'),
          dataFactory.literal('not valid json', dataFactory.namedNode('http://www.w3.org/1999/02/22-rdf-syntax-ns#JSON'))));
      });
    });
  });
});

function noop() {}
