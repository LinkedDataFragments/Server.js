...
async function collect(datasource) {
...
describe('IndexDatasource', () => {
  ...
  it('should list type, label, title, and description triples for a visible datasource with a url', async () => {
    let datasource = new IndexDatasource({
      dataFactory,
      datasources: {
    ...
    let quads = await collect(datasource);
  ...
  it('should skip a hidden datasource', async () => {
    let datasource = new IndexDatasource({
      dataFactory,
      datasources: { a: { url: 'http://example.org/a', hide: true } },
    });
    expect(await collect(datasource)).toEqual([]);
  });

  it('should skip a datasource without a url', async () => {
    let datasource = new IndexDatasource({
      dataFactory,
  ...
  it('should only list a type triple for a datasource without title or description', async () => {
    let datasource = new IndexDatasource({
      dataFactory,
