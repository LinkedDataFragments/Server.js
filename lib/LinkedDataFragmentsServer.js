...
var prototype = LinkedDataFragmentsServer.prototype = {
      ...
      'text/turtle': function () {
        q.when(queryResult).then(function (queryResult) {
          var baseURL = prefixes[''] = 'http://' + req.headers.host,
              writer = new TurtleFragmentWriter(datasourceName, baseURL + '/' + datasourceName,
                                                baseURL + req.url, pattern, self._prefixes);
          res.setHeader('Content-Type', 'text/turtle; charset=utf-8');
          writer.write(res, queryResult);
        })
        .then(null, function (error) { self.handleError(req, res, error); });
      },
    });
  },
  ...
  sendStylesheet: function (req, res) {
    ...
    fs.createReadStream(viewsFolder + 'main.css').pipe(res);
  },
