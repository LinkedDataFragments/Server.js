...
// Default routes for all Linked Data Fragments servers
var defaultRoutes = {
  respondWithFragment: /^\/([\-a-z0-9_]*)$/i,
  sendStyle: '/styles/([a-z]+)',
  sendImage: '/images/([a-z]+)',
  sendIcon: '/([a-z]+).ico',
};

// Writers of fragments per type
var fragmentWriters = {
...
function LinkedDataFragmentsServer(baseURL, datasources, options) {
  // Create the HTTP server
  var server = http.createServer(), sockets = 0;
  for (var member in prototype)
    server[member] = prototype[member];
  server._datasources = datasources || [];
  server._itemsPerPage = 100;
  server._sockets = {};
  server._options = options = _.clone(options);
  server._prefixes = _.clone(options.prefixes);
  server._dererefencePaths = _.clone(options.dereference);

  // Set up routing
  var routes = server._routes = _.clone(defaultRoutes);
  if (!_.isEmpty(server._dererefencePaths))
    routes.dereference = new RegExp('^(' + Object.keys(server._dererefencePaths)
...
var prototype = LinkedDataFragmentsServer.prototype = {
  ...
  onRequest: function (req, res) {
    ...
    res.setHeader('Server', 'Linked Data Fragments Server');
  ...
  respondWithFragment: function (req, res) {
    ...
    var query = req.query,
        ...
        page = /^[1-9][0-9]*$/.test(query.page) ? parseInt(query.page, 10) : 1,
        limit = this._itemsPerPage, offset = limit * (page - 1);

    // Create the output writer
    var self = this, host = req.headers.host,
        prefixes = _.extend(_.clone(this._prefixes), { '': formatURL(host, '/') }),
        writer = this._createFragmentWriter(req, res, { query: query, type: datasource.type,
          dataset: datasourceName, datasetURL: formatURL(host, datasourceName) + '#dataset',
          pattern: pattern, fragmentURL: formatURL(host, datasourceName, query),
          prefixes: prefixes, page: page, limit: limit, offset: offset,
        }),
    ...
    function writeEnd(error) {
      triplesWritten = true;
      if (error) return self.respondWithError(req, res, error);
      if (metadataWritten) return writer.end();
      // If metadata doesn't arrive within 5 seconds, just write a high number
      return setTimeout(function () { writeMetadata(Math.pow(2, 32)); }, 5000);
    }

    // Send the results from the datasource to the output
    datasource.query(pattern, offset, limit, writeTriple, writeMetadata, writeEnd);
  },

  // Dereferences a URL by showing its subject fragment of a certain dataset
  dereference: function (req, res) {
    var entity = formatURL(req.headers.host, req.url),
        dataset = this._dererefencePaths[req.params[0]],
        destination = formatURL(req.headers.host, '/' + dataset, { subject: entity });
  ...
  respondWithError: function (req, res, error) {
    // Halt if only an error is given
    if (!res) {
      error = req, res = req = null;
  ...
  _createFragmentWriter: function (req, res, options) {
    var defaults = { prefixes: this._prefixes,
                     rootUrl: formatURL(req.headers.host, '/'),
