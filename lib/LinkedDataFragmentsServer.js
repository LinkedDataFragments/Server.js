...
function LinkedDataFragmentsServer(options) {
  ...
  server._handlers = options.handlers || [];
...
LinkedDataFragmentsServer.prototype._handleRequest = function (request, response) {
  ...
  // Try each of the request handlers in order
  try {
    for (var i = 0; i < this._handlers.length; i++)
      if (this._handlers[i].handleRequest(request, response))
        return;
    throw new Error('No handler for ' + request.url);
  }
  // If any of the handlers failed, report the error
  catch (error) { this._sendError(request, response, error); }
};
...
LinkedDataFragmentsServer.prototype.stop = function () {
  ...
  this._handlers.forEach(function (handler) {
    try { handler.close && handler.close(); }
