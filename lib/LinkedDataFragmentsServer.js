...
function LinkedDataFragmentsServer(options) {
  ...
  server._provPath = options.provPath || '/provenance/';
  server._provMatcher = new RegExp('^' + toRegExp(server._provPath) + '(.+)|^/(\\w*)\\.ico$');
...
LinkedDataFragmentsServer.prototype._handleRequest = function (request, response) {
  ...
  // Try to serve provenance
  if (this._sendProv(request, response, writerSettings)) return;
  // Try to serve a fragment
  else if (this._sendFragment(request, response, writerSettings)) return;
  // Try to dereference
...
// Serves provenance
LinkedDataFragmentsServer.prototype._sendProv = function (request, response, writerSettings) {
  var provMatch = request.url.match(this._provMatcher);
  if (provMatch) {
    response.end('Jeej provenance');
  }
  return !!provMatch;
};
...
LinkedDataFragmentsServer.prototype._sendFragment = function (request, response, writerSettings) {
  ...
  // Add provenance link
  var provenanceUrl = url.format(_.defaults({
    pathname: this._provPath + query.datasource ,
    query: { subject: url.format(request.parsedUrl) },
  }, request.parsedUrl));
  response.setHeader('Link', '<' + provenanceUrl + '>;rel="http://www.w3.org/ns/prov#has_provenance"');
