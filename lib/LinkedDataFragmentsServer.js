...
LinkedDataFragmentsServer.prototype._handleRequest = function (request, response) {
  ...
  if (match = url.match(this._assetsMatcher))
...
LinkedDataFragmentsServer.prototype._dereference = function (request, response, datasource) {
...
LinkedDataFragmentsServer.prototype._createFragmentMetadata =
  ...
  query.patternString = '{ ' +
    ...
    (datasourceSettings.datasource.isQuadsEnabled ? (query.graph ? '<' + query.graph     + '> ' : '?g ') : '') + '}';
  ...
  return {
    datasource: _.assign(_.omit(datasourceSettings, 'datasource'), {
      ...
      templateUrl: datasourceUrl + '{?subject,predicate,object' +
                   (datasourceSettings.datasource.isQuadsEnabled ? ',graph' : '') + '}',
