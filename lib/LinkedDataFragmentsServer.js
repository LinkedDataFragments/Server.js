...
function LinkedDataFragmentsServer(options) {
  ...
  // Assign settings
  server._sockets = {};
  server._baseURL = _.omit(_.clone(url.parse(options.baseURL || "")),
...
LinkedDataFragmentsServer.prototype._createFragmentMetadata =
  ...
  var baseURL = this._baseURL,
      ...
      requestUrl = _.assign(requestUrl,
                            { protocol: (baseURL.procolol || 'http'),
                              host: (baseURL.host || request.headers.host),
                              pathname: (baseURL.pathname || "/") + requestUrl.pathname.substr(1)
                            }),
