...
function LinkedDataFragmentsServer(options) {
  ...
  if (options.protocol && options.protocol === 'https') {
    ...
    }), function (v) { return _.isArray(v) ? _.map(v, function(w) { fs.readFileSync(w); }) :  fs.readFileSync(v); }));
...
LinkedDataFragmentsServer.prototype._authenticate = function (request, response) {
    //console.log(request.connection.getPeerCertificate());
};
...
LinkedDataFragmentsServer.prototype._handleRequest = function (request, response) {
  ...
  request.parsedUrl = _.defaults(_.pick(parsedUrl, 'path', 'pathname', 'query'),
