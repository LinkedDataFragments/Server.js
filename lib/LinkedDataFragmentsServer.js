...
LinkedDataFragmentsServer.prototype._authenticate = function (request, response, callback) {
    ...
    if (certificate.subject && certificate.subject.subjectAltName) {
      ...
      this._verifyWebID(webID, certificate.modulus, certificate.exponent, function (verified) {
        if (verified)
          callback(request, response);
        else
    ...
    if (triple) {
      switch (triple.predicate) {
        ...
        case CERT_NS + 'exponent':
          var webidExponent = N3Util.getLiteralValue(triple.object);

          if (candidates[triple.subject] && webidExponent === exponent)
            verified = true;
          else
