...
function LinkedDataFragmentsServer(options) {
  ...
  if (options.protocol && options.protocol === 'https') {
    var sslOptions = _.defaults(options.ssl || {}, { key: path.join(__dirname, '../localhost.key'), cert: path.join(__dirname, '../localhost.crt') });
    server = require('https').createServer(_.mapValues(sslOptions, function (v) { return fs.readFileSync(v); }));
