...
MemoryQuadDatasource.prototype._executeQuery = function (query, tripleStream, metadataCallback) {
  ...
  // Hack to select from ALL graphs, we effectively IGNORE the query's graph
  var graphs = Object.keys(this._quadStore._graphs);
  var quads = [];
  for(var i = 0; i < graphs.length; i++) {
    quads = quads.concat(this._quadStore.findByIRI(query.subject, query.predicate, query.object, graphs[i]));
  }
  // Send the metadata
