...
// Remove an element from the quad store.
QuadStore.prototype.remove = function(element) {
  var i = this.quads.indexOf(element);
  for(var i = 0; i < this.quads.length; i++) {
    var quad = this.quads[i];
    if(isElementMatch(quad.subject, element.subject) &&
      isElementMatch(quad.predicate, element.predicate) &&
      isElementMatch(quad.object, element.object) &&
      ((!element.context) || isElementMatch(quad.context, element.context))) {
      this.quads.splice(i, 1);
      return i;
    }
  }
  console.log("Tried to remove non-occuring element.");
  return -1;
};
...
MemoryQuadDatasource.prototype.remove = function(element) {
  if(element instanceof Array) {
    ...
    this._quadStore.remove(element);
  }
};
