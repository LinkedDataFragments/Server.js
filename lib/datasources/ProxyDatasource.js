...
ProxyDatasource.prototype.supportsInsert = function (query) {
  for(var datasourceId in this._datasources) {
    var datasource = this._datasources[datasourceId];
    if(datasource.supportsInsert(query)) {
      return true;
    }
  }
  return false;
...
ProxyDatasource.prototype.insert = function (query, request, callback) {
  for(var datasourceId in this._datasources) {
    var datasource = this._datasources[datasourceId];
    if(datasource.supportsInsert(query)) {
      return datasource.insert(query, request, callback);
    }
  }
  return false;
};
