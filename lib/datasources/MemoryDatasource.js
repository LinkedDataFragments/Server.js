...
MemoryDatasource.prototype.insert = function (request, response) {
  ...
  parser.parse(function (error, triple, prefixes) {
    if(triple) {
      self._tripleStore.addTriple(triple) && amount++;
    } else {
      if(!error) {
        response.writeHead(amount === 0 ? 204 : 200, {'Content-Type': Util.MIME_PLAINTEXT});
        response.end(amount + ' new triples inserted.\n');
      } else {
        response.writeHead(400, {'Content-Type': Util.MIME_PLAINTEXT});
