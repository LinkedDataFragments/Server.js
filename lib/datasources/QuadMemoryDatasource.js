...
QuadMemoryDatasource.prototype.supportsInsert = function (query) {
  return this._insert;
};

// Adds the given triplestream to the datasource
QuadMemoryDatasource.prototype.insert = function (query, request, callback) {
  ...
  parser.parse(function (error, quad, prefixes) {
    quad ? self._quadStore.addTriple(quad) && amount++ : callback(error, amount);
  });
};
