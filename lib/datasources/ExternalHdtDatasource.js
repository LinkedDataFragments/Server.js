...
ExternalHdtDatasource.prototype._executeQuery = function (query, tripleStream, metadataCallback) {
  ...
  var hdtFile = this._hdtFile,
      hdt = spawn(hdtUtility, ['--query', (query.subject   || '?s') + ' ' +
                                          (query.predicate || '?p') + ' ' + (query.object || '?o'),
                               '--offset', query.offset, '--limit', query.limit, '--format', 'turtle',
  ...
  parser.parse(hdt.stdout, function (error, triple) {
    if (error) tripleStream.emit('error', new Error('Invalid query result: ' + error.message));
    else if (triple) tripleStream.push(triple);
    else tripleStream.push(null);
  });
  ...
  hdt.stdout.once('data', function (header) {
    metadataCallback({ totalCount: parseInt(header.match(/\d+/), 10) || 0 });
  });
