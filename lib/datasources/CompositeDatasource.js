...
function CompositeDatasource(options) {
  ...
  options.defaultGraph = options.defaultGraph || 'urn:ldf:defaultGraph';
  this._defaultGraph = options.defaultGraph;
  var allDatasources = options.datasources;
  this._datasources = {};
  this._datasourceGraphs = {};
  ...
  for (var i = 0; i < options.references.length; i++) {
    ...
    if (datasource.enabled !== false) {
      ...
      this._datasourceGraphs[datasourceName] = (this._datasources[datasourceName].settings || {}).graph || this._defaultGraph;
...
// Find a datasource's graph by datasource id inside this composition
CompositeDatasource.prototype._getDatasourceGraphById = function (datasourceIndex) {
  return this._datasourceGraphs[this._datasourceNames[datasourceIndex]];
};

CompositeDatasource.prototype._hasDatasourceMatchingGraph = function (datasource, datasourceIndex, query) {
  return !query.graph || datasource.supportedFeatures.quadPattern || query.graph === this._getDatasourceGraphById(datasourceIndex);
};
...
CompositeDatasource.prototype._getExactCount = function (datasource, query, callback) {
  ...
  var exactCount = 0;
  var outputTriples = datasource.select(emptyQuery);
  outputTriples.on('data', function () {
    exactCount++;
  });
  outputTriples.on('end', function () {
    if (exactCount > 1000)
...
CompositeDatasource.prototype._getDatasourceInfo = function (query, absoluteOffset, callback) {
  ...
  function findRecursive(datasourceIndex, offset, chosenDatasource, chosenOffset, totalCount, hasExactCount) {
    if (datasourceIndex >= self._datasourceNames.length)
      ...
      var outputTriples = datasource.select(emptyQuery);
      outputTriples.getProperty('metadata', function (metadata) {
...
CompositeDatasource.prototype._executeQuery = function (query, destination) {
  ...
  this._getDatasourceInfo(query, offset, function (datasourceIndex, relativeOffset, totalCount, hasExactCount) {
    if (datasourceIndex < 0) {
      ...
      countItems(destination, function (localEmittedCount) {
        ...
        if (emitted < limit && datasourceIndex < self._datasourceNames.length) {
          ...
          var subQuery = { offset: 0, limit: localLimit,
            subject: query.subject, predicate: query.predicate, object: query.object, graph: query.graph };
          destination.currentGraph = self._getDatasourceGraphById(datasourceIndex);
          var datasource = self._getDatasourceById(datasourceIndex);
          ...
          if (self._hasDatasourceMatchingGraph(datasource, datasourceIndex, subQuery)) {
            var outputTriples = datasource.select(subQuery);
            outputTriples.on('data', pushToDestination);
            outputTriples.on('end', closeDestination);
          }
          else
            destination.close();
          return false;
      ...
      var subQuery = { offset: relativeOffset, limit: limit,
        subject: query.subject, predicate: query.predicate, object: query.object, graph: query.graph };
      destination.currentGraph = self._getDatasourceGraphById(datasourceIndex);
      var outputTriples = self._getDatasourceById(datasourceIndex).select(subQuery);
      outputTriples.on('data', pushToDestination);
      outputTriples.on('end', closeDestination);
    }
  });
  ...
  function countItems(destination, closeCallback) {
    ...
    destination._push = function (element) {
      if (element) count++;
      if (element && (!element.graph || element.graph === selfDatasource._defaultGraph))
        element.graph = destination.currentGraph;
  ...
  function pushToDestination(triple) {
