...
Datasource.extend(SparqlDatasource, ['quadPattern', 'limit', 'offset', 'totalCount']);
...
SparqlDatasource.prototype._executeQuery = function (query, destination) {
  ...
  var sparqlPattern = this._createQuadPattern(query), self = this,
      ...
      request = { url: this._endpointUrl + encodeURIComponent(constructQuery),
        headers: { accept: 'application/x-trig;q=1.0,application/trig;q=0.95,' +
                           'text/turtle;q=0.9,application/n-triples;q=0.5,text/n3;q=0.3' },
      };

  // Fetch and parse matching triples
  (new N3.Parser()).parse(this._request(request, emitError).pipe(trigValidator()), function (error, triple) {
    if (!error) {
      if (triple)
  ...
  function trigValidator() {
    // Virtuoso sends old TriG responses, which N3.js doesn't support.
    var validator = new Transform();
    validator._transform = function (chunk, enc, cb) {
