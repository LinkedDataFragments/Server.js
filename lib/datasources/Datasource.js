...
Datasource.prototype.supportsQuery = function (query) {
  var features = query.features, feature, supports = this.supportedFeatures, hasPattern = false;
  // A query is supported if the data source supports all of its features
  if (features) {
    for (feature in features) {
      if (features[feature]) {
        if (!supports[feature]) return false;
        if (!hasPattern && feature !== 'limit' && feature !== 'offset') hasPattern = true;
      }
    }
  }
  // If no pattern was found, we have an "all" query, for which we need explicit support
  return hasPattern || !!supports.all;
};
...
Datasource.prototype.select = function (query, onError) {
  ...
  tripleStream.push = function (triple, preserveBlanks) {
    // Translate blank nodes to IRIs
    if (triple && !preserveBlanks) {
...
// Pushes the literals to the stream as triples
Datasource.prototype._pushLiterals = function (tripleStream, literals, offset) {
  for (var i = 0, id = (offset || 0) + 1; i < literals.length; i++, id++)
    tripleStream.push({ subject: '_:l' + id, predicate: RDF_VALUE, object: literals[i] }, true);
