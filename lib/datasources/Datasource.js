...
Datasource.prototype._blankNodeEnabledFields = ['subject', 'object'];
Datasource.prototype._optionalFields = [];
...
Datasource.prototype.select = function (query, onError) {
  ...
  this._blankNodeEnabledFields.forEach(function(field) {
    if (query[field] && query[field].indexOf(blankNodePrefix) === 0)
      (query = _.clone(query))[field] = '_:' + query[field].substr(blankNodePrefixLength);
  });
  ...
  tripleStream.push = function (triple) {
    ...
    if (triple) {
      self._blankNodeEnabledFields.forEach(function(field) {
        if((self._optionalFields.indexOf(field) < 0) || triple[field]) {
          if (triple[field][0] === '_') triple[field] = blankNodePrefix + triple[field].substr(2);
        }
      });
    }
    this._push(triple);
...
Datasource.prototype.fetch = function (options) {
  ...
  case 'file':
    stream = fs.createReadStream(url.substr(protocolMatch.length), { encoding: 'utf8' });
    break;
  default:
...
// Push a new element inside the datasource.
Datasource.prototype.push = function(element) {
  throw new Error("This function is not implemented for this datasource type.");
};
