...
Datasource.prototype.select = function (query, onError) {
  ...
  // Transform the received triples
  var destination = new BufferedIterator(), outputTriples, graph = this._graph;
  outputTriples = destination.map(function (triple) {
    // Translate blank nodes in the result to blank node IRIs
    if (triple.subject[0] === '_')
      triple.subject = blankNodePrefix + triple.subject.substr(2);
    if (triple.object[0]  === '_')
      triple.object  = blankNodePrefix + triple.object.substr(2);
    if (triple.graph) {
      if (triple.graph[0] === '_')
        triple.graph = blankNodePrefix + triple.graph.substr(2);
    }
