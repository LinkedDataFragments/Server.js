...
Datasource.prototype.supportsInsert = function (query) {
  return false;
};

// Adds the given triplestream to the datasource
Datasource.prototype.insert = function (query, request, callback) {
  ...
  parser.parse(function (error, triple, prefixes) {
    triple ? self.insertTriple(triple) && amount++ : callback(error, amount);
  });
};

// Adds the given triple to the datasource
Datasource.prototype.insertTriple = function (triple) {
  throw new Error('insert has not been implemented');
};
