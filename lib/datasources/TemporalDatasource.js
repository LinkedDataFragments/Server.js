...
TemporalDatasource.prototype._constructGateDatasource = function() {
  return util.instantiate(false, {
    ...
    settings: {
      timeRangeConfigs: getValues(this._timeRangeConfigs.gates).map(function(timeRange) {
        timeRange.initialRaw = Date.parse(timeRange.initial);
        timeRange.finalRaw   = Date.parse(timeRange.final);
        return timeRange;
      }),
...
TemporalDatasource.prototype._addTimeRange = function(timeRangeInterval, timeRangeConfig) {
  ...
  timeRangeConfig.settings.baseURL = this._baseURL;
...
// A query-sensitive version to get the supported features
TemporalDatasource.prototype.getSupportedFeatures = function (query) {
  var self = this;
  return attemptFeatures(query, {
    "timeRangeGate": function(q) {
      return self._gateDataource.supportedFeatures;
    },
    "timeRange": function(q) {
      ...
      return datasource.supportedFeatures;
    }}, function(q) {
    return self._staticSource.supportedFeatures;
  });
};

// Checks whether the data source can evaluate the given query
TemporalDatasource.prototype.supportsQuery = function (query) {
  var self = this;
  return attemptFeatures(query, {
    "timeRangeGate": function(q) {
      return self._gateDataource.supportsQuery(q);
    },
    "timeRange": function(q) {
      var timeRangeId = q.initial + "|" + q.final;
      var datasource = self._timeRangeDatasources[timeRangeId];
      ...
      return datasource.supportsQuery(q);
    }}, function(q) {
    return self._staticSource.supportsQuery(q);
  });
};
