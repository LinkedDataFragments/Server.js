...
View.prototype.render = function (options, request, response, done) {
  ...
  // Arrange results
  var triples = [], resultStream = settings.resultStream;
  resultStream.on('data', function (triple) { triples.push(triple); });
  resultStream.on('end',  function () { settings.triples = triples; });
  resultStream.on('metadata', function (m) { settings.metadata = m; });
