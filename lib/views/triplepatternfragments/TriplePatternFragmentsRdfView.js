...
function TriplePatternFragmentsRdfView(settings, viewNameOverride) {
  if (!(this instanceof TriplePatternFragmentsRdfView))
    return new TriplePatternFragmentsRdfView(settings);
  RdfView.call(this, viewNameOverride || 'TriplePatternFragments', settings);
}
...
TriplePatternFragmentsRdfView.prototype._generateRdf = function (settings, data, metadata, done) {
  var datasource = settings.datasource, fragment = settings.fragment, query = settings.query,
      self = this, results = settings.results, metadataDone = false;

  // Add data source metadata
  self.sendDatasourceMetadata(metadata, fragment, query, datasource);

  // Add data source controls
  self.sendDatasourceControls(metadata, fragment, query, datasource);
...
TriplePatternFragmentsRdfView.prototype.sendDatasourceMetadata = function(metadata, fragment, query, datasource) {
...
TriplePatternFragmentsRdfView.prototype.sendDatasourceControls = function(metadata, fragment, query, datasource) {
