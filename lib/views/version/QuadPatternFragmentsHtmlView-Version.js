...
function VersionHtmlViewExtension(settings) {
  ...
  HtmlView.call(this, 'QuadPatternFragments:FormAfter', settings);
}
...
VersionHtmlViewExtension.prototype._render = function (settings, request, response, done) {
  if (settings.datasource.settings.supportsVersioning)
    this._renderTemplate('version/version-form', settings, request, response, done);
  else
    done();
};
