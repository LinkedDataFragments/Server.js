...
VersionRdfViewExtension.prototype._generateRdf = function (settings, request, metadata, done) {
  ...
  if (settings.datasource.supportsVersioning) {
      ...
      Object.keys(meta.versionGraphs).forEach(function (graphName) {
        ...
        for (var i = 0; i < data.versions.length; i++) {
          metadata(
            ...
            datasource.datasourceUrl + '?version=' + data.versions[i]
          );
        }
      });
    ...
    metadata('_:versionStart', hydra + 'variable',       '"changeSetStart"');
    metadata('_:versionStart', hydra + 'property', ver + 'changeSetStart');
    metadata('_:versionEnd', hydra + 'versionEnd',       '"changeSetEnd"');
    metadata('_:versionEnd', hydra + 'versionEnd', ver + 'changeSetEnd');
