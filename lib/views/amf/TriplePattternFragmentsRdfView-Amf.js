...
AmfRdfViewExtension.prototype._generateRdf = function (settings, data, metadata, done) {
  ...
  if (settings.datasource.amf) {
    ...
    var queryVariables = (query.subject ? 1 : 0) + (query.predicate ? 1 : 0) + (query.object ? 1 : 0);

    settings.results.getProperty('metadata', function (meta) {
      if (settings.amf.enabled) {
        var maxTotalCountEnabled = 'maxTotalCount' in settings.amf.enabled ? settings.amf.enabled.maxTotalCount : 10000;
        if (query.offset || meta.totalCount >= maxTotalCountEnabled)
          return done();
      }
      ...
      if (settings.amf.inband) {
        ...
        if (!query.offset && ((queryVariables >= maxVariables) || meta.totalCount < maxTotalCount)) {
