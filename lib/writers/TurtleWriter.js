...
TurtleWriter.prototype._patternFields = ['subject', 'predicate', 'object'];
...
TurtleWriter.prototype.writeFragment = function (destination, tripleStream, settings) {
  ...
  this._patternFields.forEach(function(field) {
    writer.meta('_:triplePattern', hydra + 'mapping', '_:' + field);
  });
  this._patternFields.forEach(function(field) {
    writer.meta('_:' + field,   hydra + 'variable',      '"' + field + '"');
    writer.meta('_:' + field,   hydra + 'property', rdf + field);
  });
  ...
  // Add data triples
  var self = this;
  tripleStream.on('data', function (t) {
    // Equivalent to: writer.data(t.subject, t.predicate, t.object);
    writer.data.apply(writer, self._patternFields.map(function(field) {
      return t[field];
    }));
  });
...
TurtleWriter.prototype._createWriter = function (destination, settings) {
  var writer = new N3.Writer(settings.prefixes);
  return {
