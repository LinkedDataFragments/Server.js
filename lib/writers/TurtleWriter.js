...
TurtleWriter.prototype.writeFragment = function (destination, tripleStream, settings) {
  settings = settings || {};
  var writer = new N3.Writer(settings.prefixes), metadataWritten = false, triplesWritten = false,
  ...
  tripleStream.on('metadata', function (metadata) {
    ...
    if (query.offset)
      writeTriple(fragment.pageUrl, hydra + 'previousPage', fragment.previousPageUrl);
    ...
    triplesWritten && endWriter();
  });

  // Add data triples
  tripleStream.on('data', function (triple) { writer.addTriple(triple); });
  tripleStream.on('end',  function () { triplesWritten = true; metadataWritten && endWriter(); });
  ...
  // Ends the writer by sending the generated Turtle document
  function endWriter() {
    writer.end(function (error, turtle) { destination.end(turtle); });
  }
};
