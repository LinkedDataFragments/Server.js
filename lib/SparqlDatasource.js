...
SparqlDatasource.prototype = {
  ...
  query: function (pattern) {
    ...
    var self = this, sparqlPattern = this._createSparqlPattern(pattern);
    var request = {
    ...
    var triples = this._performRequest(request).then(parseTurtle)
    .catch(function (error) {
      ...
      if (!/Syntax error/.test(error.message)) throw error;
      request.headers.accept = 'text/ntriples';
      return self._performRequest(request).then(parseTurtle);
    });
    // return a promise to the combined triples/count object
    return q.spread([triples, this._countTotalMatches(sparqlPattern)], function (triples, count) {
  ...
  _countTotalMatches: function (sparqlPattern) {
