...
TriplePatternFragmentsController.prototype._handleRequest = function (request, response, next) {
  ...
  if  (datasourceSettings.memento) {
        mementodatetime = new Date(datasourceSettings.memento.interval[1]).toUTCString();
    ...
    timegate = datasourceSettings.timegate;
    origQuery = request.url.replace(/[^?]+/, '');
    if (timegate===true){
     timegate = this._baseurl+this._timegates.baseURL+ request.url.replace(origQuery,'').replace('/','');
     response.setHeader('Link','<' + timegate +origQuery+'>;rel=timegate');
    }
    else {
     if (timegate!==null){
     response.setHeader('Link','<' + timegate +origQuery+'>;rel=timegate');
