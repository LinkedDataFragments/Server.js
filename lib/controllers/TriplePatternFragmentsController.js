...
function TriplePatternFragmentsController(options) {
  ...
  //this._summariesEnabled = options.summaries && options.summaries.enabled;
}
Controller.extend(TriplePatternFragmentsController);

// Try to serve the requested fragment
TriplePatternFragmentsController.prototype._handleRequest = function (request, response) {
  ...
  settings.resultStream = datasourceSettings.datasource.select(query,
                          function (error) { error && response.emit('error', error); });
...
function (request, query, datasourceSettings) {
  ...
  // // Construct summaryUrl
  // var summaryUrl = this._summariesEnabled ? url.format(_.assign(_.omit(requestUrl, 'search', 'query'),
  //   { pathname: this._summariesPath + requestUrl.pathname.substring(1) })) : null;
