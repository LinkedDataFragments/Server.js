...
TriplePatternFragmentsController.prototype._handleRequest = function (request, response, next) {
  ...
  if (datasourceSettings.memento) {
    var mementodatetime = new Date(datasourceSettings.memento.interval[1]).toUTCString(),
     ...
     _.forIn(tgsettings, function (setting, key) {
       setting.versions.forEach(function (entry) {
         mementomap[entry] = key;
         origmap[entry] = setting.originalBaseURL;
       });
     });

     var origQuery = request.url.replace(/[^?]+/, ''),
         timegate = url.format(_.assign(request.parsedUrl, {pathname: this._timegates.baseURL + mementomap[query.datasource]})),
         origurl = origmap[query.datasource] || this._baseURL + mementomap[query.datasource];
     ...
     var tg = datasourceSettings.timegate,
         ogQuery = request.url.replace(/[^?]+/, '');

     if (tg === true) {
       tg = url.format(_.assign(request.parsedUrl, {pathname: this._timegates.baseURL + query.datasource})),
       response.setHeader('Link', '<' + tg + '>;rel=timegate');
     } else if (tg !== null)
