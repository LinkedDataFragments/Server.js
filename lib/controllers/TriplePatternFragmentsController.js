...
function TriplePatternFragmentsController(options) {
  ...
  this._timegateSettings = timegates.mementos || {};
...
TriplePatternFragmentsController.prototype._handleRequest = function (request, response, next) {
  ...
  if (datasourceSettings.memento) {
    var mementodatetime = new Date(datasourceSettings.memento.interval[1]).toUTCString(),
     ...
     _.forIn(this._timegateSettings, function (setting, key) {
     ...
     var origQuery = request.url.replace(/[^?]+/, ''),
         timegate = url.format(_.assign(request.parsedUrl, {pathname: this._timegateBaseUrl + mementomap[query.datasource]})),
         origurl = origmap[query.datasource] ?
                          origmap[query.datasource] + origQuery : url.format(_.assign(request.parsedUrl, {pathname: mementomap[query.datasource]}));
     ...
     if (tg === true) {
       tg = url.format(_.assign(request.parsedUrl, {pathname: this._timegateBaseUrl + query.datasource})),
