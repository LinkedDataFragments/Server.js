...
TriplePatternFragmentsController.prototype._handleRequest = function (request, response, next) {
  ...
  if (datasourceSettings.memento) {
    var mementodatetime = new Date(datasourceSettings.memento.interval[0]).toUTCString(),
        map = {};
     ...
     var origQuery = request.url.replace(/[^?]+/, ''),
         timegate = url.format(_.defaults({pathname: this._timegateBaseUrl + map[query.datasource].memento}, request.parsedUrl)),
         origurl = map[query.datasource].original ?
                          map[query.datasource].original + origQuery : url.format(_.defaults({pathname: map[query.datasource].memento}, request.parsedUrl));
     ...
     if (tg === true) {
       tg = url.format(_.defaults({pathname: this._timegateBaseUrl + query.datasource}, request.parsedUrl));
