...
function TriplePatternFragmentsController(options) {
  ...
  this._timegates = options.timegates;
...
TriplePatternFragmentsController.prototype._handleRequest = function (request, response, next) {
  ...
  if  (datasourceSettings.memento) {
      var  mementodatetime = new Date(datasourceSettings.memento.interval[1]).toUTCString(),
           tgsettings = this._timegates.mementos,       
           mementomap = {},
           origmap = {};
        Object.keys(tgsettings).forEach(function (key) {                   
              tgsettings[key].versions.forEach(function(entry){
                 mementomap[entry]=key;
                 origmap[entry]=tgsettings[key].originalBaseURL;
              });
             
         });

      var origQuery = request.url.replace(/[^?]+/, ''),
           datasource = request.url.replace(origQuery,'').replace('/',''),
           timegate = this._baseurl+this._timegates.baseURL + mementomap[datasource]+ origQuery,
           origurl = this._baseurl +  mementomap[datasource];
      if  (datasource in origmap){
          origurl =  origmap[datasource];
      }
      response.setHeader('Link','<' +origurl +origQuery +'>;rel=original, <' + timegate +'>;rel=timegate');  
      response.setHeader("Memento-Datetime", mementodatetime);
   }
  else {
      var tg = datasourceSettings.timegate;
      var ogQuery = request.url.replace(/[^?]+/, '');
      if (tg===true){     
      tg = this._baseurl+this._timegates.baseURL+ request.url.replace(ogQuery,'').replace('/','');
      response.setHeader('Link','<' + tg +ogQuery+'>;rel=timegate');
      }
      else {
      if (tg!==null){
      response.setHeader('Link','<' + tg +ogQuery+'>;rel=timegate');
