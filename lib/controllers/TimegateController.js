...
TimegateController.prototype._handleRequest = function (request, response, next) {
  ...
  if (datasource = timegateMatch && (timegateMatch[2] || timegateMatch[1])) {
    ...
    /* var parsedOriginalBaseUrl = url.parse(this._timemaps[datasource].originalBaseURL);

    console.log(parsedOriginalBaseUrl);


    var parsedOriginalBaseUrl = this._timemaps[datasource].originalBaseURL ? _.assign(request.parsedUrl, url.parse(this._timemaps[datasource].originalBaseURL)) : _.assign(request.parsedUrl, { pathname: datasource }),
        originalUrl = url.format(parsedOriginalBaseUrl);
...
TimegateController.prototype._getClosestMemento = function (timemap, accept_datetime, sorted) {
    ...
    */
    ...
    var firstMementoDatetime = new Date(firstMemento.interval[0]).getTime();
